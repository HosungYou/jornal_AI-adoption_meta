...
def select_shard(rows: list[dict[str, str]], per_family: int) -> list[dict[str, str]]:
...
def write_task_bundle(selected: list[dict[str, str]], source_packet_dir: Path) -> None:
    rows = []
        ...
        rows.append(
            {
                ...
                "previous_locked_output_excluded": "true",
                "notes": "Selected deterministically from post-freeze task shell after excluding prior locked outputs and generic beta/path exception rows.",
            }
        )
    write_csv(
        TASK_ID_OUTPUT,
...
def write_preflight(selected: list[dict[str, str]], source_packet_dir: Path) -> None:
...
def main() -> None:
    ...
    selected = select_shard(task_rows, per_family=30)
    write_task_bundle(selected, DEFAULT_SOURCE_PACKET_DIR)
    write_preflight(selected, DEFAULT_SOURCE_PACKET_DIR)
