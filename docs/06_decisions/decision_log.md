...
|------|----------|-----------|------------------------|--------|
| 2026-06-11 | **Paper B canonical reference and model framing fixed for Step 5** | OneDrive `Paper2_Human_Final_Consensus_20260605_v2` is the canonical human consensus package, while Git Step 4/5 files are downstream source-audited freeze, task-unit, locked-output, and scoring layers. Paper B needs denominator-family validity and downstream substitution evidence, not a single overall task-unit accuracy score. | **Use raw coder returns directly:** loses source-audited adjudication and freeze boundaries. **Collapse 8,783 task units into one denominator:** obscures task difficulty and error consequence. **Rank vendors in the main text:** drifts from workflow validation and ages poorly. | S072 ANX-EE `r = 1.0` is trace/influence diagnostic only; `source_blank_r` direct-r rows stay in primary direct-r extraction; converted beta/path/source-statistic rows enter numeric extraction evaluation as explicit strata; Codex GPT-5.5 is the primary workflow; Claude/Gemini are supplementary cross-model sensitivity; Claude Sonnet now has clean `0000-7858` model-explicit coverage after 2026-06-11 backfill, while legacy Claude Code/default-unspecified rows remain audit provenance only; final SEM claims remain sample-size-blocked despite R/OpenMx/metaSEM readiness. |
| 2026-04-25 | **Extraction workspace simplified into numbered workflow folders** | The prior `data/04_extraction` folder mixed protocols, coder packages, consensus workbooks, reference placeholders, and archived screening/progress files at the same level. Numbered folders make the five-step workflow explicit and reduce accidental use of the wrong artifact. | **Keep flat structure:** faster but confusing and error-prone. **Delete old workbooks:** simpler but risks losing raw audit evidence. | Active extraction files now follow `00_protocol` through `05_llm_masem_substitution`; duplicate coder-package manual DOCX files were removed; raw coder workbooks were preserved. |
| 2026-04-25 | **Phase 1+2 combined Paper B validation corpus adopted** | Phase 1 and Phase 2 differ mainly by time block and reviewer-pair composition. Raw human disagreement should be analyzed before consensus, but LLM accuracy and MASEM substitution require a frozen source-anchored adjudicated human reference standard. | **Phase 1 primary + Phase 2 optional:** underuses the rotated-pair data and makes the validation corpus unnecessarily small. **No adjudicated reference:** preserves raw disagreement but leaves no defensible target for LLM accuracy or substitution analysis. | Paper B reports Phase 1+2 together, includes pre-adjudication human disagreement analysis, avoids `gold standard` terminology, and uses the adjudicated reference for LLM comparison. Current Phase 2 load is R1/R4 = 57 studies (+7), R2/R3 = 56 studies (+6). |
| 2026-04-24 | **Phase 2 rotated-pair human coding adopted** (R1+R4, R2+R3) | Phase 1 pairwise coding is complete. Rotating pairs reduces pair-specific bias and preserves a human-first reference standard before LLM comparison. | **Continue Phase 1 pairs:** risks carrying pair-specific habits forward. **AI-first single verification:** faster but weaker for LLM augmentation validation and blinding. | Superseded in scope by the 2026-04-25 combined-corpus decision. Phase 2 now contributes to both Paper A extraction and Paper B validation. |
...
| 2026-03-04 | **Model 4 (Trust Dual-Mechanism) added** as 4th competing model | Trust's effect on BI may be partially mediated through ATT (Reliance proxy) and ANX (Resistance suppression proxy), not only direct. Conceptual framework discussion (2026-02-27) identified Trust's dual mechanism as theoretically important. Approach B (competing model comparison) confirmed. | **3-model only:** Misses Trust mediation mechanism. **Add Reliance/Resistance as new constructs:** Likely k<5 for these variables, insufficient data. **Use existing ATT/ANX as proxies:** Feasible within current 12-construct framework. | Extends Model 2 with 2 paths (TRU→ATT, TRU→ANX); enables testing Trust's total effect decomposition; contingency: drop Model 4 if TRU↔ATT or TRU↔ANX pair k<5 |
| 2026-03-04 | **Approach B (Competing Model Comparison) confirmed** for MASEM Stage 2 | Avoids HARKing by pre-specifying 4 competing models before data analysis; enables rigorous model comparison via Δχ², ΔCFI, AIC/BIC; aligns with OSF preregistration requirements | **Approach A (purely confirmatory):** Too rigid for 12-construct exploratory landscape. **Approach C (post-hoc models):** HARKing risk, methodologically weaker. | All 4 models must be pre-registered on OSF before Stage 2 analysis begins |
