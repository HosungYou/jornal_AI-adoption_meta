...
COMPLETE_DIR = REPO_ROOT / "data/04_extraction/05_llm_masem_substitution/results/paper_a_source_corrected_complete_case_20260614"
PARTIAL_DIR = REPO_ROOT / "data/04_extraction/05_llm_masem_substitution/results/paper_a_anx_tru_rescue_masem_20260614"
ANX_DIR = REPO_ROOT / "data/04_extraction/05_llm_masem_substitution/results/paper_a_anx_tru_rescue_after_source_correction_20260614"
OUT_DIR = REPO_ROOT / "data/04_extraction/05_llm_masem_substitution/results/paper_a_model_family_masem_submission_packet_20260614"
ONEDRIVE_DIR = Path("/Users/newhosung/Library/CloudStorage/OneDrive-SharedLibraries-ThePennsylvaniaStateUniversity/AI Adoption Meta Analysis - Documents/Meta/AI Adoption/04_analysis_strategy/Paper_A/2026-06-14_model_family_masem_submission_packet")
...
def main() -> None:
    ...
    complete_summary = read_csv(COMPLETE_DIR / "paper_a_source_corrected_complete_case_summary_20260614.csv")
    partial_summary = read_csv(PARTIAL_DIR / "paper_a_masem_execution_summary_20260614.csv")
    coverage = read_csv(ANX_DIR / "paper_a_source_corrected_plus_anx_tru_coverage_20260614.csv")
    ...
    eligibility.to_csv(OUT_DIR / "paper_a_model_family_eligibility_table_20260614.csv", index=False)
    write_md_table(eligibility, OUT_DIR / "paper_a_model_family_eligibility_table_20260614.md")

    fit_frames = []
    for route in ["paper_a_core7_att_mediation", "paper_a_trust6_mechanism"]:
        path = COMPLETE_DIR / f"{route}_complete_case_stage2_fit_indices_20260614.csv"
        fit = read_csv(path)
    ...
    fit_table.to_csv(OUT_DIR / "paper_a_model_family_fit_table_20260614.csv", index=False)
    write_md_table(format_df(fit_table), OUT_DIR / "paper_a_model_family_fit_table_20260614.md")

    path_frames = []
    for route in ["paper_a_core7_att_mediation", "paper_a_trust6_mechanism"]:
        p = COMPLETE_DIR / f"{route}_complete_case_stage2_paths_20260614.csv"
        paths = read_csv(p)
    ...
    path_table.to_csv(OUT_DIR / "paper_a_model_family_structural_paths_20260614.csv", index=False)
    write_md_table(path_table, OUT_DIR / "paper_a_model_family_structural_paths_20260614.md")

    coverage.to_csv(OUT_DIR / "paper_a_full10_coverage_after_anx_tru_rescue_20260614.csv", index=False)
    write_md_table(coverage.fillna(""), OUT_DIR / "paper_a_full10_coverage_after_anx_tru_rescue_20260614.md")
    ...
    md = f"""# Paper A Model-Family MASEM Submission Packet
...
- Full10 pairwise coverage after ANX-TRU rescue: {int(full10_row['observed_pairs_after_rescue'])}/{int(full10_row['required_pairs'])} pairs.
...
"""
    (OUT_DIR / "PAPER_A_MODEL_FAMILY_MASEM_SUBMISSION_PACKET_20260614.md").write_text(md, encoding="utf-8")
