...
input_path <- Sys.getenv(
  ...
  unset = file.path(repo, 'data/04_extraction/05_llm_masem_substitution/results/paper_a_source_corrected_s004_s048_20260614/paper_a_source_corrected_s004_s048_input_20260614.csv')
)
out_dir <- Sys.getenv(
  'PAPER_A_COMPLETE_CASE_OUT_DIR',
  unset = file.path(repo, 'data/04_extraction/05_llm_masem_substitution/results/paper_a_source_corrected_complete_case_20260614')
)
onedrive_out <- '/Users/newhosung/Library/CloudStorage/OneDrive-SharedLibraries-ThePennsylvaniaStateUniversity/AI Adoption Meta Analysis - Documents/Meta/AI Adoption/03_source_adjudication/Paper_A/2026-06-14_priority15_detailed_source_review'
...
run_route <- function(route_name, spec) {
  ...
  if (nrow(eigen_rows) > 0) {
    ...
    write_csv(eigen_rows, file.path(out_dir, paste0(route_name, '_complete_case_matrix_eigen_20260614.csv')))
  }
  ...
  if (length(built$mats) >= 2) {
    ...
    if (!is.null(fit1)) {
      saveRDS(fit1, file.path(out_dir, paste0(route_name, '_complete_case_tssem1_fit_20260614.rds')))
      ...
      if (inherits(fit2, 'error')) {
        ...
        write_csv(paths, file.path(out_dir, paste0(route_name, '_complete_case_stage2_paths_20260614.csv')))
        write_csv(fit_indices, file.path(out_dir, paste0(route_name, '_complete_case_stage2_fit_indices_20260614.csv')))
        saveRDS(fit2, file.path(out_dir, paste0(route_name, '_complete_case_tssem2_fit_20260614.rds')))
...
summary_path <- file.path(out_dir, 'paper_a_source_corrected_complete_case_summary_20260614.csv')
...
report_path <- file.path(out_dir, 'PAPER_A_SOURCE_CORRECTED_COMPLETE_CASE_TSSEM_20260614.md')
writeLines(report, report_path)
file.copy(summary_path, file.path(onedrive_out, 'paper_a_source_corrected_complete_case_summary_20260614.csv'), overwrite = TRUE)
file.copy(report_path, file.path(onedrive_out, 'PAPER_A_SOURCE_CORRECTED_COMPLETE_CASE_TSSEM_20260614.md'), overwrite = TRUE)
