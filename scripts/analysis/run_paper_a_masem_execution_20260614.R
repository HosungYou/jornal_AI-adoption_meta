...
default_input_path <- file.path(repo, 'data/04_extraction/05_llm_masem_substitution/results/paper2_masem_substitution_rerun_input_n_weighted_all_source_supported_20260612.csv')
input_path <- Sys.getenv('PAPER_A_MASEM_INPUT', unset = default_input_path)
out_dir <- Sys.getenv('PAPER_A_MASEM_OUT_DIR', unset = file.path(repo, 'data/04_extraction/05_llm_masem_substitution/results/paper_a_masem_execution_20260614'))
...
run_route <- function(route_name, spec) {
  ...
  write_csv(pair_cov, file.path(out_dir, paste0(route_name, '_pair_coverage_20260614.csv')))
  pooled <- pairwise_pool(raw, construct_vec, route_name)
  write_csv(pooled, file.path(out_dir, paste0(route_name, '_pairwise_random_effects_stage1_20260614.csv')))
  ...
  if (missing_pairs == 0 && !is.na(min_eigen) && min_eigen <= 0) {
    ...
    write_csv(as.data.frame(as.table(near_mat)) %>% rename(construct_1 = Var1, construct_2 = Var2, nearpd_r = Freq) %>% mutate(route = route_name, .before = 1), file.path(out_dir, paste0(route_name, '_nearpd_matrix_20260614.csv')))
  }
  write_csv(as.data.frame(as.table(pooled_mat)) %>% rename(construct_1 = Var1, construct_2 = Var2, pooled_r = Freq) %>% mutate(route = route_name, .before = 1), file.path(out_dir, paste0(route_name, '_pairwise_pooled_matrix_20260614.csv')))
  ...
  if (!is.null(fit1)) {
    saveRDS(fit1, file.path(out_dir, paste0(route_name, '_tssem1_fit_20260614.rds')))
    ...
    if (inherits(fit2, 'error')) {
      ...
      write_csv(path_rows, file.path(out_dir, paste0(route_name, '_stage2_paths_20260614.csv')))
      write_csv(fit_rows, file.path(out_dir, paste0(route_name, '_stage2_fit_indices_20260614.csv')))
      saveRDS(fit2, file.path(out_dir, paste0(route_name, '_tssem2_fit_20260614.rds')))
    }
  }
...
write_csv(summaries, file.path(out_dir, 'paper_a_masem_execution_summary_20260614.csv'))

report <- c(
  '# Paper A MASEM Execution Attempt', '', 'Date: 2026-06-14', '',
...
writeLines(report, file.path(out_dir, 'PAPER_A_MASEM_EXECUTION_ATTEMPT_20260614.md'))
